...
const STEPS = [
  ...
  { id: 2, label: 'Bridge', icon: '🌉' },
  { id: 3, label: 'Concept Psy', icon: '🧠' },
  { id: 4, label: 'Mécanique', icon: '🎬' },
  { id: 5, label: 'Scores', icon: '⭐' },
...
function App() {
  ...
  const canGoNext = () => {
    switch (step) {
      case 1: return config.fond.trim().length > 10
      case 2: return config.bridge.trim().length > 5
      case 3: return config.conceptPsy !== null
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 2: return <StepBridge config={config} updateConfig={updateConfig} />
      case 3: return <StepConceptPsy config={config} updateConfig={updateConfig} />
      case 4: return <StepMecanique config={config} updateConfig={updateConfig} />
      case 5: return <StepScores config={config} updateConfig={updateConfig} />
    ...
    <div className="min-h-screen bg-bg-primary">
      <header className="border-b border-border px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <span className="text-text-muted text-sm">FOND × BRIDGE × CONCEPT × MÉCANIQUE</span>
        </div>
      </header>
